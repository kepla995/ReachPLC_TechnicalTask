...
describe('Technical Task for ReachPLC - F companies', () => {
  ...
  beforeEach(() => {
    ...
    cy.get('.browse').contains('F').click()
  })
  it('Business Details for Fannin UK / Kent Pharma UK Limited', () => {

    //click on 1st Company.
    cy.get('.row').contains('Fannin UK / Kent Pharma UK Limited').click()
    ...
    .each(function($detail, index, $details){ //Function
      cy.wrap($detail).within(function(){
        ...
        cy.get('.col-md-3').screenshot('Fannin UK Kent Pharma UK Limited')
        ...
        cy.writeFile('./cypress/CompanyDetails/Fannin UK Kent Pharma UK Limited.json', $detail.text().trim())

      })
    })
  })

  it('Business Details for Ferndale Pharmaceuticals Ltd', () => {

    //click on 3rd Company.
    cy.get('.row').contains('Ferndale Pharmaceuticals Ltd').click()
    ...
    .each(function($detail, index, $details){ //Function
      cy.wrap($detail).within(function(){
        ...
        //Get screenshot of Company Logo
        cy.get('.col-md-3').screenshot('Ferndale Pharmaceuticals Ltd')
        ...
        cy.log($detail.text().trim())
        cy.writeFile('./cypress/CompanyDetails/Ferndale Pharmaceuticals Ltd.json', $detail.text().trim())

      })
    })
  })

    it('Business Details for Fresenius Kabi Ltd', () => {

      //click on last Company.
      cy.get('.row').contains('Fresenius Kabi Ltd').click()
      ...
      .each(function($detail, index, $details){ //Function
        cy.wrap($detail).within(function(){
          ...
          //Get screenshot of Company Logo
          cy.get('.col-md-3').screenshot('Fresenius Kabi Ltd')
          ...
          cy.log($detail.text().trim())
          cy.writeFile('./cypress/CompanyDetails/Fresenius Kabi Ltd.json', $detail.text().trim())
