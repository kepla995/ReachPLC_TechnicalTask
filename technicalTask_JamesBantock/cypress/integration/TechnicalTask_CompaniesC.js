...
describe('Technical Task for ReachPLC - C companies', () => {
  ...
  beforeEach(() => {
    ...
    cy.get('#onetrust-accept-btn-handler').contains('Accept All Cookies').click({force: true})
      cy.get('.browse').contains('C').click()
  })
  it('Business Details for Camurus AB', () => {

    //click on 1st Company for A.
    cy.get('.row').contains('Camurus AB').click()
    ...
    .each(function($detail, index, $details){ //Function
      cy.wrap($detail).within(function(){
        ...
        cy.get('.col-md-3').screenshot('Camurus AB')
        ...
        cy.writeFile('./cypress/CompanyDetails/Camurus AB.json', $detail.text().trim())

      })
    })
  })

  it('Business Details for Celgene Ltd', () => {

    //click on 3rd Company for A.
    cy.get('.row').contains('Celgene Ltd').click()
    ...
    .each(function($detail, index, $details){ //Function
      cy.wrap($detail).within(function(){
        ...
        //Get screenshot of Company Logo
        cy.get('.col-md-3').screenshot('Celgene Ltd')
        ...
        cy.log($detail.text().trim())
        cy.writeFile('./cypress/CompanyDetails/Celgene Ltd.json', $detail.text().trim())

      })
    })
  })

    it('Business Details for CSL Behring UK Limited', () => {

      //click on last Company for A.
      cy.get('.row').contains('CSL Behring UK Limited').click()
      ...
      .each(function($detail, index, $details){ //Function
        cy.wrap($detail).within(function(){
          ...
          //Get screenshot of Company Logo
          cy.get('.col-md-3').screenshot('CSL Behring UK Limited')
          ...
          cy.log($detail.text().trim())
          cy.writeFile('./cypress/CompanyDetails/CSL Behring UK Limited.json', $detail.text().trim())
